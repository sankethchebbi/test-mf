...
from sankethmf import Sankethmf
...
# Predefined password (you can use a more secure way to manage passwords)
VALID_PASSWORD = "truebeacon"

# Set up a session state variable to store whether the user is authenticated
...
if not st.session_state.authenticated:
    ...
    st.set_page_config('TB MF', layout="wide")


    # Define tabs
    tabs = st.tabs(tabs=['TB analysis'])
    ...
    with tabs[0]:
        st.title("TB MF Analysis")

        analysis_tabs = st.tabs(
            tabs=['Analysis Mode', 'Comparison Mode',  'Quick Tools', 'Detailed Annual Returns'])
        ...
        with analysis_tabs[0]:  # Analysis Mode
            ...
            benchmark_ticker = st.text_input("Benchmark Ticker (Yahoo Finance)", "^NSEI")
            ...
            if fund_code:
                ...
                if not nav_df.empty and not benchmark_df.empty:
                    ...
                    with st.expander(f"Show NAV Data and Financial Metrics for {scheme_name}", expanded= True):
        ...
        with analysis_tabs[1]:  # Comparison Mode
            ...
            selected_scheme_names = st.multiselect(
                ...
                default=[scheme_names[4608]] if scheme_names else []  # Default selection
            )
            ...
            benchmark_ticker_comparison = st.text_input("Benchmark Ticker for Comparison (Yahoo Finance)", "^NSEI")
            st.write('Get Benchmark Ticker names from Yahoo Finance.')
